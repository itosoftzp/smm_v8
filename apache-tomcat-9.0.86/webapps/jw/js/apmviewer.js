...
APMNode = function() {
    ...
    this.addTransaction = function(found, transaction) {
        ...
        var key = found[1];
        if (found[0].indexOf('/embed/') !== -1) {
            key = "embed_" + key;
        }
        
        if (found.length > 2) {
            if (this.transactions["CHILD_OF_" + key] === undefined) {
                this.transactions["CHILD_OF_" + key] = new APMNode().initBase(found[0], found[1], [], this);
            }
            ...
            this.transactions["CHILD_OF_" + key].addTransaction([found[0], found[2]], transaction);
        } else {
            if (this.transactions[key] === undefined) {
                this.transactions[key] = new APMNode().initBase(found[0], found[1], [], this);
            }
        
            this.transactions[key].updateNode(transaction);
            if (this.transactions[key].parent !== null && this.transactions[key].parent !== undefined) {
                this.transactions[key].isTransition = true;
            }
        }
    };
    this.reset = function() {
    ...
    this.renderDetails = function(detailrow) {
        ...
        if (this.detailrow === null) {
            ...
            APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['average'] + this.transactionParam + APMViewer.getTimeRange() + APMViewer.getNode(), function(data){
                    ...
                    tooltip: {
                        ...
                        formatter: function (params) {
                            ...
                            for (var i = 0; i < params.length; i ++) {
                                ...
                                html += "<tr><td>"+params[i].marker+" <strong>"+data.dataSeries[i].name+"</strong></td><td style=\"text-align:right\">"+value+"</td></tr>";
                            }
                            html += "</table>";
                            return html;
                        },
            ...
            APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['percentiles'] + this.transactionParam + APMViewer.getTimeRange() + APMViewer.getNode(), function(data){
            ...
            APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['throughput'] + this.transactionParam + APMViewer.getTimeRange() + APMViewer.getNode(), function(data){
    ...
    this.slowtraceDetails = function() {
        ...
        APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['points'] + this.transactionParam + APMViewer.getTimeRange() + APMViewer.getNode(), function(data){
    ...
    this.errorsDetails = function() {
        ...
        APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['errpoints'] + thisObj.transactionParam + APMViewer.getTimeRange() + APMViewer.getNode(), function(data){
            APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['messages'] + thisObj.transactionParam + APMViewer.getTimeRange() + APMViewer.getNode(), function(data2){
    ...
    this.queriesDetails = function() {
        ...
        APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['queries'] + this.transactionParam + APMViewer.getTimeRange() + APMViewer.getNode(), function(data){
    ...
    this.servicesDetails = function() {
        ...
        APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['service'] + this.transactionParam + APMViewer.getTimeRange() + APMViewer.getNode(), function(data){
    ...
    this.threadDetails = function() {
        ...
        APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['profile'] + this.transactionParam + APMViewer.getTimeRange() + APMViewer.getNode(), function(data){
    ...
    this.showTraceView = function(traceId) {
        var thisObj = this;
        APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['trace'] + traceId + APMViewer.getNode(), function(data){
            $("#traceview").html("");
            
            var html = "<h3 class=\"boxy-content-header\">" + data.headline + " <a style=\"color: #ffe100;\" href=\""+APMViewer.contextPath + APMViewer.urls['export'] + traceId + "?" + APMViewer.getNode().replace('&', '') + "\" target=\"_blank\"><i class=\"fas fa-cloud-download-alt\"></i></a></h3><div class=\"boxy-content-body trace-info\">";
            ...
            if (data.entryCount > 0) {
                ...
                $("#traceview").find('.trace-entries a').on("click", function() {
                    if ($("#traceview").find('.trace-entries .content').length === 0) {
                        APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['traceEntries'] + traceId + APMViewer.getNode(), function(data){
            ...
            if (data.queryCount > 0) {
                ...
                $("#traceview").find('.trace-query a').on("click", function() {
                    if ($("#traceview").find('.trace-query .content').length === 0) {
                        APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['traceQueries'] + traceId + APMViewer.getNode(), function(data){
            ...
            if (data.mainThreadProfileSampleCount > 0) {
                ...
                $("#traceview").find('.trace-profile a').on("click", function() {
                    if ($("#traceview").find('.trace-profile .content').length === 0) {
                        APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['traceProfile'] + traceId + APMViewer.getNode(), function(data){
...
APMViewer = {
    urls : {
        ...
        'apps' : '/web/json/console/monitor/app/list',
        'export' : '/web/json/console/monitor/apm/export/trace/',
    },
    deferreds : [],
    contextPath : '',
    totalMemory : 0,
    ...
    init : function(contextPath, totalMemory, maxheap, title, isVirtualHostEnabled, appId, base64Authentication, node) {
        ...
        APMViewer.base64Authentication = base64Authentication;
        APMViewer.node = node;
    ...
    deletData : function() {
        APMViewer.httpPost(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['removeData'] + APMViewer.getNode(), {}, function(){
    ...
    loadAlertList : function(callback) {
        var thisObj = this;
        APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['config'] + APMViewer.getNode(), function(data){
            ...
            $("#alertpopup .apm-alert").find("table .rowaction.delete").on("click", function() {
                var thisRowObj = $(this);
                var id = $(this).data("id");
                APMViewer.httpPost(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['removeAlert'] + APMViewer.getNode(), {
                    "version" : id
                },
                function(data){
    ...
    loadAlertDetail : function(id) {
        ...
        if (id === undefined) {
            ...
            APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['config'] + "&version=" + id + + APMViewer.getNode(), function(data){
        ...
        $.when.apply($, deferreds).then(function() {
            var options = {
                ...
                saveCallback: function(container, properties) {
                    ...
                    APMViewer.httpPost(APMViewer.contextPath + APMViewer.urls['base'] + action + APMViewer.getNode(), data,function(data){
                        APMViewer.loadAlertList();
                    });
                }
            };
    ...
    smtpView : function() {
        ...
        APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['smtp'] + APMViewer.getNode(), function(data){
            var propertyValues = {
            ...
            var options = {
                ...
                saveCallback: function(container, properties) {
                    ...
                    APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['adminGeneral'] + APMViewer.getNode(), function(data){
                        if (data.config.agentDisplayName !== APMViewer.title) {
                            data.config.agentDisplayName = APMViewer.title;
                            APMViewer.httpPost(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['adminGeneral'] + APMViewer.getNode(), data.config, function(data){
                                //ignore
                            });
                        }
                    });
                    ...
                    APMViewer.httpPost(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['smtp'] + APMViewer.getNode(), newdata, function(data){
    ...
    sendTestMail : function(properties) {
        ...
        APMViewer.httpPost(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['sendTestEmail'] + APMViewer.getNode(), properties, function(data){
            if (data.error) {
    ...
    loadAppsList: function (data) {
        ...
        $.each(data.data, function (i, v) {
            if (APMViewer.node !== undefined && APMViewer.node !== "") {
                APMViewer.apps[v.id] = new APMNode().initBase(v.name, get_apmviewer_msg('apm.app') + " : " + v.name, ['^.+/web/userview/' + v.id + '(/[^/]+)(.*)$', '^.+/web/embed/userview/' + v.id + '(/[^/]+)(.*)$', '^.+/web(/ulogin)/' + v.id + '(/.*)$']);
            } else {
                if (APMViewer.contextPath.indexOf('http') !== -1) {
...
//        APMViewer.apps['WEBCONSOLE'] = new APMNode().initBase("Web Console", get_apmviewer_msg('apm.webConsole'), ['^.+'+APMViewer.contextPath+'(/)$', '^.+'+APMViewer.contextPath+'(/web)$', '^.+'+APMViewer.contextPath+'(/home.*)$', '^.+'+APMViewer.contextPath+'(/web/console/.*)$', '^.+'+APMViewer.contextPath+'(/web.*)$']);
        if (APMViewer.node !== undefined && APMViewer.node !== "") {
            APMViewer.apps['OTHERS'] = new APMNode().initBase("Others", get_apmviewer_msg('apm.others'), ['^.+/(.*)$']);
        } else {
            if (APMViewer.contextPath.indexOf('http') !== -1) {
    ...
    loadUserviewList: function (data) {
        ...
        $.each(data, function (i, v) {
            ...
            if (APMViewer.node !== undefined && APMViewer.node !== "") {
                APMViewer.apps[v.id] = new APMNode().initBase(v.name, displayName, ['^.+/web/userview/' + APMViewer.appId + '/' + v.id + '(.*)$', '^.+/web/embed/userview/' + APMViewer.appId + '/' + v.id + '(.*)$']);
            } else {
                if (APMViewer.contextPath.indexOf('http') !== -1 && (APMViewer.node === undefined || APMViewer.node === "")) {
    ...
    loadGaugesChart : function() {
        ...
        APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['gauges'] + APMViewer.getTimeRange() + APMViewer.getNode(), function(data){
            for (var i = 0; i < data.dataSeries.length; i++) {
                data.dataSeries[i].type = "line";
                ...
                tooltip: {
                    ...
                    formatter: function (params) {
                        ...
                        for (var i = 0; i < params.length; i ++) {
                            ...
                            html += "<tr><td>"+params[i].marker+" <strong>"+data.dataSeries[i].tlabel+"</strong></td><td style=\"text-align:right\">"+value+"</td></tr>";
                        }
                        html += "</table>";
                        return html;
                    },
    ...
    loadSummary : function() {
        ...
        APMViewer.httpGet(APMViewer.contextPath + APMViewer.urls['base'] + APMViewer.urls['summary'] + APMViewer.getTimeRange() + appIdParam + APMViewer.getNode(), function(data){
    ...
    getNode : function() {
        if(APMViewer.node !== undefined && APMViewer.node !== ""){
            return "&node=" + encodeURIComponent(APMViewer.node);
        } else{
            return "";
        }
    },
    httpGet : function(url, callback) {
        ...
        if (APMViewer.base64Authentication !== null && APMViewer.base64Authentication !== undefined && APMViewer.base64Authentication !== "") {
            ...
            $.ajax({
               ...
               beforeSend: function (request) {
                  ...
                  request.setRequestHeader("Authorization", "Basic " + APMViewer.base64Authentication);
               },
               xhrFields: {
                        ...
                        format : function(value, obj) {
                            ...
                            if (obj.name.match(new RegExp('^.+'+APMViewer.contextPath+'/web/userview/')) || 
                                ...
                                if (!obj.isTransition) {
                                    ...
                                    if (obj.name.indexOf('/embed/') !== -1) {
                                        value += ' (' + get_apmviewer_msg('apm.embed') + ')';
                                    }
                                }
                                if (value === "/offline" || value === "/pwaoffline" || value === "/_/cacheUrls" || value === "/_/serviceworker" || value === "/manifest") {
                                    return value;
                                } else {
