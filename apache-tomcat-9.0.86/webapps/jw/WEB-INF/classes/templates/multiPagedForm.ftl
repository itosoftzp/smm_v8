<div class="form-cell" ${elementMetaData!}>
    ...
    <div id="${elementParamName!}_multiPagedForm_${element.properties.elementUniqueKey!}" class="form-element multiPagedForm ${element.properties.displayMode!}">
        ...
        <input type="hidden" class="errorPage" name="${elementParamName!}_errorPage" value="${errorPage!?html}" />
            ...
            <#list element.children as e>
                <#if e.className == "org.joget.plugin.enterprise.MultiPagedFormChild" >
                    ...
                    <#if cPageNum! == e.properties.pageNum!>
                        ...
                        <li class="nav_item current ${errorCss}"><button rel="${e.properties.pageNum!}" disabled><span>${e.properties.label!?html}</span></button></li>
                    <#else>
        ...
        <div class="page-container">
            <#list element.children as e>
                <#if e.className == "org.joget.plugin.enterprise.MultiPagedFormChild" >
                    <#if cPageNum! == e.properties.pageNum!> 
                        <div class="page page_${e.properties.pageNum!} current" ${e.getAjaxData(formData)!}>
                            ${e.render(formData, includeMetaData!false)}
                        </div>
                    <#else>
                        <div class="page page_${e.properties.pageNum!}" style="display:none;" ${e.getAjaxData(formData)!}>
                            ${e.render(formData, includeMetaData!false)}
                        </div>
                    </#if>
                <#else>
        ...
        <div class="page-button-panel">
            <input type="submit" class="page-button-prev btn button" name="${elementParamName!}_prev_page" value="${element.properties.prevButtonlabel!?html}" <#if cPageNum! == "1">disabled</#if> />
            <input type="submit" class="page-button-next btn button" name="${elementParamName!}_next_page" value="${element.properties.nextButtonlabel!?html}" <#if elementMetaData! != "">disabled</#if> <#if cPageNum! == element.properties.totalPage!>disabled</#if>/>
        </div>

    </div>
    <script type="text/javascript">
        $(document).ready(function(){
            $("#${elementParamName!}_multiPagedForm_${element.properties.elementUniqueKey!}").multiPagedForm(${options!});
        });
    </script>
</div>