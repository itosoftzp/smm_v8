	...
	this.element.disableSelection().bind('click.pq-scrollbar',function(evt){
		...
		if(that.direction=="vertical"){
			...
			var botSlider=topSlider+ that.$slider.width();
                        ...
                        var right = parseInt(that.$slider[0].style.right.replace("px", ""));
			if(top<topSlider){
                                if ($("body").hasClass("rtl")) {
                                    that.$slider.css("right", right + (topSlider-top));
                                } else {
				...
				that._updateCurPosAndTrigger(evt);
                                ...
                                if ($("body").hasClass("rtl")) {
                                    that.$slider.css("right", right - (top - botSlider));
                                } else {
	...
	this.$slider = $("div.pq-sb-slider",this.element).draggable({axis:axis,
		...
		drag:function(evt){
                    if ($("body").hasClass("rtl")) {
                        var cLeft= evt.pageX;
                        var right = parseInt(that.$slider[0].style.right.replace("px", ""));
                        var newRight = right + (that.dragLeft - cLeft);
                        var scroll_space = that.length - 34 - that.$slider[0].offsetWidth;
                        if (newRight < 17) {
                            newRight = 17;
                        } else if (newRight > 17 + scroll_space) {
                            newRight = 17 + scroll_space;
                        }
                        that.$slider.css("right", newRight);
                        that.dragLeft = cLeft;
                    }
			that.dragging=true;
...
fnSB._updateCurPosAndTrigger = function(evt,top){
	...
	if(top==null){
		top=(that.direction=="vertical")?parseInt($slider[0].style.top):(($("body").hasClass("rtl"))?parseInt($slider[0].style.right):parseInt($slider[0].style.left));
	}
...
(function ($) {
    ...
    _pG._generateTables = function (objP) {
		...
		var GM=thisOptions.groupModel,
			hidearrHS1 = [];
        {   
            var modifiedColumn = false;
            // Calculate whether an extra column is needed, and calculate the width for the first column.
            if (that.options.notEmptySpaceScroll) {
		var grid = $(".pq_grid");
                modifiedColumn = true;
                var cur_pos = that.$hscroll.pqScrollBar("option", "cur_pos");
                var count = 0;
                var totalColWidth = 0;
                var trimmedColModel = thisColModel.slice(1, -1);
                //length in RTL mode will need to retrieve from the grid and deduct the row count column if there is any
                var length = that.$hscroll.pqScrollBar("option", "length");
                if ($("body").hasClass("rtl")) {
                ...
                for (var c = count - 1; c >= 0; c--) {
                    if (length < (pW + Math.floor(trimmedColModel[c].width))) {
                        columnArray.push(c + 1);
                    }
                    pW += Math.floor(trimmedColModel[c].width);
                }

                //calc remaining column width
                var rW = 0;
                var addExtraColumn = false;
                var extraColumnWidth = 0;
                //loop to get remaining width 
                for (var c = count - 1; c >= 0; c--) {
                    if (length > (rW + Math.floor(trimmedColModel[c].width))) {
                        rW += Math.floor(trimmedColModel[c].width) + 1;
                    } else {
                         //add 1 more column to see if fit into the grid width 
                        if ((length - (rW + Math.floor(trimmedColModel[c].width))) > -10) {
                            rW += Math.floor(trimmedColModel[c].width) + 1;
                        } else {
                            // Stop the loop
                            break;
                    ...
                    extraColumnWidth = trimmedColModel[c].width;
                }

                // Check if there is available space for an extra column.
                // If yes, add one more column in the row and calculate the margin left for table.
                // Else, calculate the width for the first column.
		var mL = 0;
                if (length - rW > 0) {
                    addExtraColumn = true;
                    mL = (rW + Math.floor(extraColumnWidth) + 1) - length; // margin-right for the table
                    if (cur_pos !== 0) {
                        buffer = ["<table class='" + tblClass + "' cellpadding=0 cellspacing=0 style='margin-left: -" + mL + "px;'>"];
                    }
                } else if (length - rW < 0) {
                    extraColumnWidth = length - rW - 1;// 
                }
                ...
                var hiddenColumns = [];
                var lastAddedColumnIndex = -1;
                //generate hidearrHS based on the columnArray
                for (var col = 0; col < noColumns; col++) {
                    if ($.inArray(col, columnArray) !== -1) {
                        hiddenColumns.push(true);
                        if (addExtraColumn) {
                            lastAddedColumnIndex = hiddenColumns.length - 1; // Update the index
                        }
                    //if there is extra column set the item to false
                    } else if (lastAddedColumnIndex !== -1) {
                        hiddenColumns[lastAddedColumnIndex] = false;
                        lastAddedColumnIndex = -1; // Reset the index
                    } else {
                        hiddenColumns.push(false);
                    }
                }
                that.hidearrHS = hiddenColumns;
                hidearrHS1 = columnArray;
                
                //no modified the column if current position is 0
                if (cur_pos === 0) {
			...
			for (var col = 0; col < noColumns; col++) {
                ...
                var wd = 0;
                // change the first column width, other follow the same
                if (modifiedColumn) {
                    if (addExtraColumn) {
                        wd = extraColumnWidth;
                    } else {
                        wd = outerWidths[col] + extraColumnWidth;
                    }
                    modifiedColumn = false;
                } else {
                    wd = outerWidths[col];
		...
		window.setTimeout(function(){
	        ...
	        that._trigger("refresh", null, {
                ...
                that.$tbl.css("width", that.$tbl.closest(".pq_grid").find(".pq-grid-header-table").outerWidth());
		},0);
    }
	...
	_pG._generateRow=function(rowData ,rowIndx,thisColModel,noColumns,hidearrHS1,offset,const_cls,buffer,objP){
        ...
        // Similar to line 710, calculate to see if an extra column is needed.
	var addExtraColumn = false;
	var mL = 0;
        if (that.options.notEmptySpaceScroll) {
            var grid = $(".pq_grid");
            ...
            var mL = 0;
            //calculate margin-right for table
            if (length - rW > 0) {
                addExtraColumn = true;
                mL = (rW + Math.floor(extraColumnWidth)) - length + 1; 
            } 
            ...
            for (var col = 0; col < noColumns; col++) {
                if ($.inArray(col, columnArray) !== -1) {
                    ...
                    // Modify the previous element to false
                    hiddenColumns[lastAddedColumnIndex] = false;
                    lastAddedColumnIndex = -1; // Reset the index
                } else {
                    hiddenColumns.push(false);
            ...
            //not change if current position is 0;
            if (cur_pos === 0) {
        ...
        //fixed the numbering column to left when extra column is added
        if (that.numberCell) {
            if (addExtraColumn) {
                buffer.push("<td style='width:" + (that.numberCellWidth + 1)  + "px;position: relative;left: " + mL + "px;top: 0px;height: 100%;' class='pq-grid-number-cell ui-state-default'>\
        ...
        for (var k = 0; k < hidearrHS1.length; k++) {
			...
			objRender.colIndx=col;					
            var strStyle = "";
            strStyle += "visibility:hidden;border:none;";
            var cls = const_cls; 
            if (column.align == "right") {
                cls += ' pq-align-right';
            } else if (column.align == "center") {
    ...
    fn._createHeader = function () {
        ...
        //calculate the header and set position
        var reset = false;  
        var tW = 0;
        var pW = 0;
        if (thisOptions.notEmptySpaceScroll) {
            var grid = $(".pq_grid");
            var count = 0;
            var totalColWidth = 0;
            var cur_pos = this.$hscroll.pqScrollBar("option", "cur_pos");
            var trimmedColModel = optColModel.slice(1, -1);
            ...
            count = count + cur_pos;
            //no reset the row if current position is 0
            if (cur_pos === 0) {
                reset = true;
                lft = 0;
            }

            for (let j = 0; j < this.outerWidths.length - 1; j++) {
                if (j > count) {
                    //get width of the current total column Width
                    tW = this.outerWidths[j] + tW;
                } else {
                    //get width of the previous total column Width
                    pW = this.outerWidths[j] + pW;
                }
            }
        }
        //set position left or right based on the condition
        if (this.options.direction == "rtl") {
            if (thisOptions.notEmptySpaceScroll) {
                if (that.scrollMode && !reset) {
                    $header_right.css('right', '');
                    $header_right.css({left: "-" + tW + "px"});
                } else {
                    $header_right.css('left', '');
                    $header_right.css({right: "-" + lft + "px"});
                }
            } else {
                $header_right.css({right: "-" + lft + "px"});
            }
