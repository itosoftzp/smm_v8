...
AjaxComponent={currentUrlEventListening:[],initAjax:function(element){AjaxComponent.unbindEvents();$(element).find("[data-ajax-component], [data-ajax-component][data-events-triggering], [data-ajax-component][data-events-listening]").each(function(){AjaxComponent.initContent($(this));});setTimeout(function(){AjaxComponent.triggerPageLoadedEvent();AjaxComponent.triggerEvents($("#content.page_content"),window.location.href,"get");},2);setTimeout(function(){$(window).trigger('resize');},5);},initContent:function(element){AjaxComponent.overrideLinkEvent(element);setTimeout(function(){AjaxComponent.overrideButtonEvent(element);AjaxComponent.overrideDatalistButtonEvent(element);AjaxComponent.overrideFormEvent(element);AjaxComponent.initEventsListening(element);if(window["AdminBar"]!==undefined){AdminBar.initQuickEditMode();}},1);},overrideLinkEvent:function(element){setTimeout(function(){$(element).on("click","a[href]:not(.off_ajax)",function(e){var a=$(this);var href=$(a).attr("href");var target=$(a).attr("target");var onclick=$(a).attr("onclick");if((typeof PwaUtil==='undefined'||PwaUtil.isOnline!==false)&&onclick===undefined&&AjaxComponent.isCurrentUserviewUrl(href)&&!AjaxComponent.isDatalistExportLink(a)&&(target===null||target===undefined||target===""||target==="_top"||target==="_parent"||target==="_self")){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();AjaxComponent.call($(a),href,"GET",null);return false;}
...
let id;$(window).resize(function(){let firstItemLength;let menuItemId=$(navItems[0]).prop('id');if($(window).outerWidth()>=menuBreakpoint){firstItemLength=$("#"+menuItemId).outerWidth();}
if(firstItemLength!==navItemWidth[0]&&$(window).outerWidth()>=menuBreakpoint){navItemWidth.length=0;$('#category-container > li').each(function(){let itemWidth=$(this).outerWidth();navItemWidth.push(itemWidth);});}
...
function formatNav(containerWidth){let room=true;let count=0;let tempWidth=0;let totalWidth=0;let navPadding=$('#category-container').outerWidth()-$('#category-container').width();let numItems=navItems.length-1;if(containerWidth===undefined){containerWidth=$('#navigation').innerWidth();}
navItems.each(function(){tempWidth=totalWidth+navItemWidth[count]+navPadding;if($(window).outerWidth()<menuBreakpoint){$('#menu-more').before($('#moreSubMenu').children().first());$('#menu-more').hide();}
